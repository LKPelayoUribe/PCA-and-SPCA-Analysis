...
keywords: |
    pca, svm
abstract: |
  \noindent This research paper studies cancer prediction from gene expression data using PCA and SPCA methods
...
* The second data set used in this analysis was the Spambase Data Set, which is also hosted by the UC Irvine Machine Learning Repository. This data set has 4,601 emails which are described by 57 real-valued features, each of which is labeled (tagged) asspam or non-spam.
...
PCA was applied to the data resulting in 801 principal components in total (one for each sample). The variance explained by each of the principal components, ordered by successively large eigenvalues, was also computed and plotted to demonstrate the cumulative variance explained by each of the 801 principal components. A plot of the first two principal component scores was also created. Looking at the visualization of the scores for the first two components class clusters can be seen clearly. KIRC and COAD appear to be the most distinct, with the other 3 classes, BRCA, LUAD, and PRAD, having a significant amount of overlap. Both of these plots can be seen below in **Figure 1** and **Figure 2**:

### TODO: Explain Eigenvectors **Figure 3**
...
The first 8 principal components (of 801) calculated at $\approx 50\% of the total explained variance indicating that a significant amount of the information that characterizes the data is present in relatively few principal components. Oberservation:

* The cancer gene score plot visulaized clusters for each of the cancer classes. KIRC and COAD gene samples were the most distinct clusters of all the gene classes, having little overlap with other classes. 
...
Comparing the results between PCA and SPCA, we see that even though the first 8 sparse PCs account for less $\approx 43\%$of the variance explained, which was less than that of classical PCA at $\approx 50\%$ for the same number of components, the score plots are still very similar. Bot the PCA anbd SPCA, we can see clear clusters of class labels can be seen with KIRC samples being the most distinctly different. 
...
* There were no genes among the top 10 loadings that are common between the first three principal components in PCA. In order to determine this, the top 10 absolute values of the loadings for PC 1, 2, and 3, were extracted from the PCA calculation. We then did a intersect between PCA (1, 2), (1, 3), and (2, 3), to see if the same genes were present in the top 10 loadings for each principal component. No common genes were found among the first 3 PC for PCA (See Appendix for R code)
...
In this section, PCA was applied to email data to determine whether emails can be reliably classified as spam or non-spam. The first step of this section was to load the data and remove the testid column which contained sample labels predicted during a previous analysis. The data was checked for missing values and none were found. 
...
### TODO: Explain Eigenvectors **Figure 9**
...
spca = elasticnet::spca(stdgexpProj2[,2:1001], K=6, type="predictor", sparse="penalty", lambda=1e-6, para=rep(1e-6, 18), max.iter=200, eps.conv=1e-3)
